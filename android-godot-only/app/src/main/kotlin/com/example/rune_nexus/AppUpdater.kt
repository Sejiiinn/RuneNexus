...
class AppUpdater(private val activity: Activity) {
    ...
    fun perform(method: String, args: Map<String, Any>, result: Result) {
            val changesFile = method == "downloadUpdate" || method == "downloadPatch" || method == "installUpdate"
            if (changesFile && !updateInProgress.compareAndSet(false, true)) {
                result.error("update_in_progress", "Application update already running")
                return
            }
            ...
            try {
                when (method) {
                    ...
                    "downloadUpdate" -> {
                        val url = requireNotNull(args["url"] as? String)
                        val hash = requireNotNull(args["sha256"] as? String)
                        val size = requireNotNull(args["sizeBytes"] as? Number).toLong()
                        val expectedVersion = requireNotNull(args["versionCode"] as? Number).toLong()
                    ...
                    "downloadPatch" -> {
                        try {
                            ...
                            val size = requireNotNull(args["sizeBytes"] as? Number).toLong()
                            val expectedVersion = requireNotNull(args["versionCode"] as? Number).toLong()
                            val targetHash = requireNotNull(args["targetSha256"] as? String)
                            val targetSize = requireNotNull(args["targetSizeBytes"] as? Number).toLong()
                    ...
                    "installUpdate" -> {
                        ...
                        activity.runOnUiThread {
                            try {
                                ...
                                result.error("update_install_failed", "Unable to open Android installer")
                            } finally {
                                updateInProgress.set(false)
                            }
                        }
                    }
                    else -> result.error("invalid_operation", "Unsupported update operation")
                }
            } catch (_: SignatureMismatchException) {
                result.error("update_signature_mismatch", "Update must use the installed application signing key")
            } catch (_: Exception) {
                result.error("update_failed", "Unable to download or verify application update")
            } finally {
