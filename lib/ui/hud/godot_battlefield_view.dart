...
class _GodotBattlefieldViewState extends State<GodotBattlefieldView>
  ...
  void initState() {
    ...
    widget.game.nativeSessionClock = true;
    widget.game.nativeHostChanges.addListener(_requestSync);
  ...
  void _setLoading(bool loading) {
    final changed = widget.game.nativeBattlefieldLoading != loading;
    widget.game.nativeBattlefieldLoading = loading;
    if (changed) _requestSync();
  ...
  Future<void> _pollStatus() async {
    ...
    try {
      ...
      _combatSupported =
          status?['nativeCombatVersion'] == 1 &&
          status?['nativeSessionVersion'] == 1;
      if (status?['ready'] == true && !_combatSupported) {
        throw StateError('Native session protocol version 1 is required');
      }
  ...
  void didChangeDependencies() {
    ...
    final blocked = !TickerMode.valuesOf(context).enabled;
    if (widget.game.nativeSessionUiBlocked != blocked) {
      widget.game.nativeSessionUiBlocked = blocked;
      _requestSync();
    }
    final settings =
  ...
  void _requestSync() {
    ...
    scheduleMicrotask(() {
      ...
      if (!_syncRunning) {
        _syncQueued = false;
        unawaited(_pumpSession());
      }
    });
  }

  Future<void> _pumpSession() async {
  ...
  String _frameFingerprint(Map<String, Object?> frame) {
    final state = Map<String, dynamic>.from(
      jsonDecode(jsonEncode(frame)) as Map,
    );
    for (final key in [
      'seq',
      'time',
    ...
    final effects = presentation?['effects'] as Map?;
    for (final key in ['clock', 'squaredSteps', 'shake']) {
      effects?.remove(key);
    }
    final selection = presentation?['selection'] as Map?;
    for (final key in ['clock', 'time', 'aim']) {
      selection?.remove(key);
    }
    // Authoritative Godot combat supplies dynamic core/enemy labels.
    presentation?.remove('labels');
  ...
  Future<void> _syncFrame() async {
    ...
    final generation = _syncGeneration;
    if (!game.readyNotifier.value) return;
    ...
    var submittedSequence = _sequence;
    ...
    encoded['inputBlocked'] =
        !widget.inputEnabled ||
        game.paused ||
        game.nativeSessionUiBlocked ||
        game.nativeBattlefieldLoading;
    if (game.nativeRewardViewport != null) {
      encoded['rewardViewport'] = game.nativeRewardViewport;
    }
    final fingerprint = _frameFingerprint(encoded);
    final unchanged =
        _available &&
        _lastApplied >= _lastSubmitted &&
        fingerprint == _lastFrameFingerprint;
    _sending = true;
    if (unchanged) {
      submittedSequence = _lastSubmitted;
    } else {
      _sequence++;
      _lastSubmitted = submittedSequence;
      _lastFrameFingerprint = fingerprint;
    }
    try {
      final json = unchanged
          ? await _channel.invokeMethod<String>('getPresentation', {
              'sceneEpoch': epoch,
            })
          : await _channel.invokeMethod<String>('submitFrameV2', {
              'sceneEpoch': epoch,
              'frame': jsonEncode(encoded),
            });
  ...
  Future<void> _syncCombat() async {
    ...
    try {
      final command = game.buildNativeSessionCommand(epoch);
      if (command != null) _combatPendingSince ??= now;
      final json = command == null
          ? await _channel.invokeMethod<String>('getSessionState', {
              'sceneEpoch': epoch,
            })
          : await _channel.invokeMethod<String>('submitCombat', {
              'sceneEpoch': epoch,
      ...
      if (json != null && json.isNotEmpty) {
        ...
        game.applyNativeCombatResponse(response);
        if (!game.nativeCommandPending) _combatPendingSince = null;
      }
      ...
      if (epoch == _sceneEpoch && generation == _syncGeneration) {
        _combatSending = false;
      }
    }
  }
  ...
  void didUpdateWidget(covariant GodotBattlefieldView oldWidget) {
    ...
    if (oldWidget.game != widget.game) {
      oldWidget.game.nativeHostChanges.removeListener(_requestSync);
      widget.game.nativeSessionClock = true;
      widget.game.nativeSessionUiBlocked = !TickerMode.valuesOf(
        context,
      ).enabled;
      widget.game.nativeHostChanges.addListener(_requestSync);
    ...
    if (oldWidget.inputEnabled != widget.inputEnabled) _requestSync();
  ...
  void dispose() {
    ...
    _statusTimer?.cancel();
    _syncTimer?.cancel();
    widget.game.nativeHostChanges.removeListener(_requestSync);
    final game = widget.game;
    final epoch = _sceneEpoch;
