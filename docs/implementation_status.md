...
마지막 갱신 기준: `main` 브랜치 커밋 `d194812`.

## 요약

Rune Nexus는 Flutter + Flame 기반의 플레이 가능한 로그라이트 타워 디펜스 MVP 단계다. 기본 전투 루프, 젬/링크, 포탑 성장, 런 한정 젬 파편, 저장/복구, 스테이지 선택/해금, 결과 화면, 룬 기반 영구 업그레이드 기초가 구현되어 있다.
...
- 챕터는 5개 스테이지 단위로 구분
  ...
  - 챕터 2: 스테이지 6~10
- 스테이지 1~10별 맵 정의 분리
- 스테이지 1 기본 웨이브, 스테이지 2~5 장갑병 중심 웨이브, 스테이지 6~10 보호막병 중심 웨이브 분리
- 스테이지 1 기본 해금
...
- 코어 패시브 `자가 수복` 장착 시 5라운드마다 Nexus HP 1 회복
- 코어 패시브 `연산 가속` 장착 시 전투 스킬 재사용 대기시간 10% 감소
- 코어 전투 스킬 `균열 낙인` 장착 시 챕터 2부터 체력이 높은 적에게 받는 피해 증가 낙인 부여
- 자동 타겟팅
- 투사체 이동과 충돌 판정
- 단일 피해
...
- 적 6종
  ...
  - 보스
- 장갑병은 방어구 내구도 계위를 사용
- 보호막병은 보호막과 보호막 재생 계위를 사용하며, 챕터 2인 스테이지 6~10 웨이브에 등장
- 적 저항 프로필
...
- 스테이지 6~10 전용 보호막병 중심 50라운드 웨이브
