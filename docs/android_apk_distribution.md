...
앱 시작과 백그라운드 복귀 시 업데이트를 확인합니다. `minimumSupportedVersionCode`보다 낮은 앱은 필수 업데이트를 완료해야 게임을 시작·재개할 수 있습니다. 최소 버전 이상이면 선택 업데이트를 건너뛸 수 있습니다. 조회 실패·오프라인에서는 필수 여부를 확인할 수 없어 재시도만 제공합니다. 복귀 검사 중 기존 게임 상태는 유지하고 입력과 화면 갱신을 차단합니다. 서버가 `CLIENT_UPDATE_REQUIRED`를 반환한 경우에도 로그인·저장을 유지한 채 `업데이트하기` 버튼 하나로 기존 앱 내 다운로드·설치 흐름에 연결합니다. 서버 필수 판정에서는 선택 업데이트의 건너뛰기를 제공하지 않으며, 실패 시 같은 버튼이 `다시 시도`로 바뀝니다. 웹에서는 업데이트 버튼으로 페이지를 새로고침합니다.
