...
앱 시작과 백그라운드 복귀 시 업데이트를 확인합니다. `minimumSupportedVersionCode`보다 낮은 앱은 필수 업데이트를 완료해야 게임을 시작·재개할 수 있습니다. 최소 버전 이상이면 선택 업데이트를 건너뛸 수 있습니다. 조회 실패·오프라인에서는 필수 여부를 확인할 수 없어 재시도만 제공합니다. 복귀 검사 중 기존 게임 상태는 유지하고 입력과 화면 갱신을 차단합니다.
...
- **실제 APK 기준 비교:** 파일 바이트 크기와 증가량·증가율을 기록합니다. APK ZIP 항목의 압축 후 크기를 기준으로 `lib/<ABI>/`, Flutter 에셋, Godot PCK의 기여도를 구분합니다. 소스 폴더 크기나 빌드 옵션만 보고 절감됐다고 판단하지 않습니다.
...
- **에셋 중복:** 같은 3D 자산이 Flutter의 원본 GLB·텍스처와 Godot PCK 안에 함께 들어가는지 확인합니다. 플랫폼별 실제 로딩 경로와 프리뷰 사용 여부를 확인한 뒤 불필요한 포함을 정리합니다. `design/`의 편집 원본·중간 렌더가 배포에 섞이지 않도록 합니다.
