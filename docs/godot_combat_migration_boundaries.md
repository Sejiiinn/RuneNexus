...
순수 계산은 [AttackCalculation](../lib/domain/combat/attack_calculation.dart), 기존 컴포넌트와의 연결은 [CombatResolver](../lib/game/systems/combat_resolver.dart), Godot 대응 계산은 [attack_calculation.gd](../godot/combat/attack_calculation.gd)에 둔다. Godot 계산은 비교 실행용이며 본게임의 전투 생산자로 연결하지 않았다.
...
연구·모듈 인벤토리와 코어 스킬의 활성 상태는 앱에서 입력으로 제공한다. 적별 특성 스택, 연쇄 정리 타이머의 진행, 타깃 선택과 발사 시점·치명타 난수 생성은 기존 전투 책임으로 남긴다. 연쇄 정리의 공속 보정은 현재 활성 여부만 계산 입력으로 사용한다. 저장 모델·보상·경제 API는 변경하지 않는다.

[TurretStatCalculation](../lib/domain/combat/turret_stat_calculation.dart)은 [읽기 전용 입력과 발사 시 고정 입력](../lib/domain/combat/turret_stat_input.dart)을 사용한다. 실제 포탑 getter는 최신 앱 보정값을 읽는 어댑터에 연결하고, 발사 시에는 불변 입력을 한 번 캡처해 기존 `TurretAttackSnapshot` 필드로 옮긴다. 일반 getter에서 스탯 전체를 계산하거나 젬 집합을 매번 복제하지 않는다. [Godot 대응 계산](../godot/combat/turret_stat_calculation.gd)은 아직 비교 실행용이며 본게임 전투 명령에 연결하지 않았다.
...
이 묶음도 Android 본게임이나 실기기 성능 검증을 뜻하지 않는다. 실제 전투 책임은 Dart/Flame에 유지하며, 타깃·적 상태·특성 발동 상태를 연결해 전환하기 전까지 Godot 계산은 비교용으로 둔다.
...
웨이브 생성·코어 스킬 타이머·보상·경제·앱 UI는 아직 Dart가 담당한다. 코어 스킬의 실제 적 피해는 명령으로 전달하고 처치·코어 도달 결과는 한 번만 앱에 반영한다. 기존 저장 모델에 맞는 스냅샷 미러는 과도기 연결이며, 엔진 간 상태 전달의 최종 최적화나 Flame 의존성 제거 완료를 뜻하지 않는다. 미지원 스테이지·플랫폼의 기존 2D 전투도 남아 있다.
