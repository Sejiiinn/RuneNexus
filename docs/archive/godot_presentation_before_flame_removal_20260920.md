> 역사 기록: 표시 단위별 이관 당시의 경계다. 구현과 검증 수치는 기록 당시 기준이다.
...
지원되는 Android 3D 전장은 초기 상태의 인수 응답 뒤 적 이동·상태·피해, 포탑 타깃·발사, 탄환·번개 연쇄와 웨이브 생성·코어 스킬 주기·웨이브 완료 판정을 Godot이 담당한다. 적·포탑·탄환 표시와 적 내구도 라벨도 Godot 상태로 생성하며 해당 Flame 갱신은 중지한다. 앱은 정적 웨이브 계획, 배속·정지 명령, 코어 피격의 방어·패배 처리, 보상·경제·UI·저장을 담당한다. 명령/응답·저장 경계와 검증 범위는 [전투 이관 문서](../godot_combat_migration_boundaries.md)를 따른다. 전투 인수 후 오류는 전투를 정지시키며, 표시 fallback만으로 Flame 전투를 재가동하지 않는다.

Godot은 전장 모델과 전장에 붙는 정보·선택·효과를 같은 카메라로 표시한다. Flutter는 로비·HUD·패널·보상 카드·교체 UI와 화면 전체 피격 경고를 유지한다. 기존 2D 경로에서는 Dart/Flame이 전투와 웨이브를 계속 처리한다. 아래 이벤트 전송 설명은 표시 전용 호환 경로에도 적용되며, native 전투가 생산하는 모델·피해·코어 효과는 Godot 내부 상태를 사용한다. 저장 스키마와 전투 규칙은 유지한다. 과거 표시 수명 이관 범위와 검증은 [수명 이관 기록](../../design/stage1_3d/presentation_migration/native_lifecycle/README.md)을 따른다.
...
아래 표의 Dart 이벤트·좌표 생산 설명은 표시 전용 호환 경로 기준이다. 전투 인수 후 적·포탑·탄환·코어 스킬의 대응 입력 생산자는 Godot 전투 런타임이며, 앱 UI·보상 입력은 Dart에서 유지한다.
...
SequentialLightningChainComponent (`../../lib/game/components/sequential_lightning_chain_component.dart`, 당시 경로)는 지연·다음 타깃·명중을 수행하므로 유지한다. Projectile/Enemy/Turret도 그림의 소유권만 구분한다.
