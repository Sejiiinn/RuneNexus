...
> 상태 원본: Godot 전환의 구현·검증·미완료 판정은 이 문서의 [단계별 상태](#3-단계와-의존-관계)에서 관리한다. UI 보존·정리 기준은 [복원 기준표](../godot_ui_restoration_baseline.md), 개별 검증 결과는 단계별 근거 기록을 따른다.

역할: 현재 남은 Flutter/Dart 의존성을 대체하고 제거하는 순서·대상·완료 조건.
작성: 2026-09-20. 수정: 2026-09-21. 구현 기준: `2a8eb37` (실제 콘텐츠·런·성장 이관).
상태: 전환 진행 중. 독립 경로의 대체 구현과 일반 Android 본게임의 실제 교체를 구분한다. 단계별 완료 범위는 3절을 따른다.
...
아래 표는 **일반 Android 본게임 경로**의 책임이다. 독립 개발 경로의 구현 상태는 다음 절에서 구분한다.
...
현재 상태의 근거는 [전투 책임](../godot_combat_migration_boundaries.md), [표시·앱 연결](../godot_presentation_migration_plan.md), [NativeGameHost](flutter_reference_20260924.md), [전투 앱 연결](flutter_reference_20260924.md)이다. 기존 문서의 과거 Flutter+Flame 설명을 새 이관의 출발점으로 삼지 않는다.

<a id="현재-두-실행-경로와-확정한-전환-방식"></a>

### 현재 실행 경로와 확정한 전환 방식

**Godot 독립 경로에 기능을 단계적으로 구현·검증하고, 저장·UI·계정·패키징 등 전환 조건을 갖춘 뒤 본게임 진입 경로를 교체한다.** 새로 이관한 기능마다 Flutter 본게임에 임시 브리지를 추가하는 방식은 기본 경로로 삼지 않는다. 기존 Godot 전투·3D 자산은 공유하며, 기능별 동등성 검증과 최종 앱 통합 검증을 나누어 진행한다.

| 구분 | 일반 Android 본게임 | 독립 `--session` | 독립 `--app` |
| --- | --- | --- | --- |
| 진입 | 기존 Flutter 앱 | 개발·회귀 검증 세션 | 앱 수명주기·UI 검증 경로 |
| 전투·표시 | 기존 Godot 전투 기반 | 같은 기반 | 같은 기반 |
| 런·성장 | Dart 도메인 | Godot 도메인 | Godot 도메인 |
| 로컬 저장 | 기존 Flutter/Dart 경로 | 수동 Save/Load·Exit 저장 | 자동 저장·시작 시 정지 복원 |
| UI | 기존 Flutter UI | 검증용 조작 UI | Godot UI; 동등성은 3·5단계에서 판정 |
| 계정·배포 | 기존 서비스·패키지 | 인증·업로더 미연결 | 인증·업로더 미연결; 별도 Android 검수 패키지는 본게임 교체가 아님 |

실행 방법·검수 패키지는 [인앱 가이드](../../.agents/in_app_test_guide.md#독립-godot-앱세션), 격리 저장과 검증 근거는 [앱 수명주기 기록](../analysis/godot_app_ui_20260921/README.md), 개발 세션 명령은 [세션 안내](../../godot/session/README.md)를 따른다. 구현 범위와 남은 관문은 아래 각 단계에서 관리한다.

따라서 최근 런·성장 이관은 독립 경로의 대체 구현 완료다. 일반 Android 앱이 해당 Godot 도메인을 사용하도록 전환된 상태는 아니다. 기존 앱의 Dart 도메인은 전환 전까지 유지하며, 소비자가 실제로 사라진 뒤 제거한다. 독립 경로의 데스크톱 전투 검증을 Android 저장 인계·계정·배포 검증으로 대신하지 않는다.
...
단계 번호는 남은 Flutter 제거의 완료 관문이다. 3과 4는 2의 공통 상태·저장 서비스가 갖춰지면 독립적으로 진행할 수 있다. 완료된 Godot 전투를 미착수로 되돌리지 않는다. 독립 경로에서 기능별 대체 구현을 검증하고, 본게임 진입 전환 때 실제 소비자가 대체된 책임부터 제거한다. 독립 구현 완료만으로 기존 본게임의 호출·저장 경로를 끊지 않는다.
...
상태: **진행 중**. v1/v2 호환 codec, 네이티브 로컬 저장·중단 복구, 전투 체크포인트 투영 및 고정 설정 fixture 세션의 프로세스 재시작 검증을 구현했다. [저장 기반 검증](../analysis/godot_save_foundation_20260921/README.md). 독립 세션의 실제 콘텐츠·전투 설정 경로도 구현·검증했다([콘텐츠 이관 기록](../analysis/godot_content_migration_20260921/README.md)). 정적 정의는 Dart 단일 원본에서 생성 JSON으로 전달하며 실행/준비에는 Dart가 필요 없고 재생성 도구 의존은 남아 있다. 독립 세션의 건설·강화·판매·젬·특성·런 업그레이드, 일반 성장·연구·코어·장착과 전투 이벤트 반영도 Godot에서 처리한다([런·성장 검증](../analysis/godot_run_growth_20260921/README.md)). 실제 런 v2 Save/Load·Exit 저장과 별도 프로세스 복원도 구현·검증했다([실제 런 저장·복원 기록](../analysis/godot_content_save_20260921/README.md)). 준비·웨이브·구매/일반 보상 대기·실패·성공과 저장 손상·재시도·종료 실패 경계를 헤드리스로 확인했으며, 사용자 요청에 따라 이번 시각 검증은 생략했다. 후속으로 퀘스트·런 종료 진행·영속 보상 큐와 정산 호출 경로를 구현했다([정산·퀘스트 구현 기록](../analysis/godot_rewards_quests_20260921/README.md)). 독립 `--app`에 자동 저장·시작 시 정지 복원을 연결했다([앱 수명주기·UI 기록](../analysis/godot_app_ui_20260921/README.md)). Android 본게임의 기존 Flutter 호출, 인증/온라인 저장 업로더·설치 경로 인계는 남아 있다. 웹 저장 어댑터는 실제 브라우저 검증 전이다.
...
상태: **독립 경로의 주요 HUD·상태별 조작 복구, 본게임 제거는 미완료**. 후속 [Flutter 기준 HUD 복구](../analysis/godot_hud_restore_20260921/README.md)에서 기본/포탑/젬/보상/결과의 구조·기존 이미지와 빠진 조작을 복구하고 Android 대표 상태를 검수했다. `--app`에서 Godot 자원·웨이브·전투 조작·건설·포탑/젬/강화·보상·결과 UI를 사용한다. 기존 이미지·한글 폰트와 전투 명령을 재사용한다. 기존 Flutter HUD의 본게임 소비자는 유지하며, 성능 동등성/최종 제거 관문과 계정 서비스는 별도다. [구현·검증 기록](../analysis/godot_app_ui_20260921/README.md).

후속 [메뉴 흐름·표현 정렬](../analysis/godot_flutter_menu_audit_20260922/implementation.md)에서 홈 메뉴의 목적지·취소 동작, 전투 상세 정보와 로컬 메뉴 차이를 보완했다. headless 회귀 결과이며 이번 변경의 시각적 동등성은 미검증이다.
...
상태: **일부 구현, 전체 단계 미완료**. 계정 정산은 실제 HTTP 요청 경로·동일 key/본문 바이트 재시도·writer/revision 동기화 선행·snapshot 적용과 저장·단조 증가 캐시를 구현했다. 인증된 context와 실제 저장 업로드 성공을 반환하는 `sync_save`를 `app.settle_pending_rewards(context, sync_save, transport)`에 주입해야 한다. 전체 로그인·온라인 저장 업로더·자동 계정 스케줄링과 실계정 E2E는 미완료이며 운영 서버는 호출하지 않았다. [정산·퀘스트 구현 기록](../analysis/godot_rewards_quests_20260921/README.md)을 따른다.
...
상태: **일부 구현, 전체 단계 미완료**. 로비와 성장 화면의 명령 연결·전투 왕복·기기 설정은 [앱 UI 기록](../analysis/godot_app_ui_20260921/README.md)에 있다. 후속 [메인 로비 복구](../analysis/godot_lobby_restore_20260921/README.md)에서 홈의 Flutter 기준 구조·외형과 설정/이벤트 모달·이어하기 진입을 검증했다. [하위 화면 복구](../analysis/godot_subpages_restore_20260921/README.md)에서 스테이지·강화·연구·코어·모듈·일일/주간 임무의 기존 게임 컴포넌트·배치와 로컬 명령 연결을 복구하고 Android 대표 화면 및 자동 회귀를 확인했다. 원본 기준은 [복원 기준표](../godot_ui_restoration_baseline.md)에 있다. 로그인·뽑기·분해·퀘스트 수령·우편·리더보드는 미연결이다. 일반 본게임 로비 제거·기존 설치 데이터 인계·정식 진입 교체도 남아 있다.
...
상태: **미완료**. [Flutter 미포함 Godot 전용 검수 APK](../analysis/fps_godot_only_20260921/README.md)를 별도 Android 호스트로 빌드했다. 기존 독립 검수 패키지와 구분하며, 기존 설치 인계·정식 패키징·본게임 진입 교체의 완료 근거는 아니다.
...
상태: **미완료**. 일반 본게임의 Flutter/Dart 소비자가 유지되므로 최종 제거 조건을 충족하지 않았다.
...
다음 구현은 4단계의 인증·온라인 저장과 서버 경제 연결을 우선으로, 이에 의존하는 5단계의 계정·우편·리더보드 등 서비스 화면과 이후 정식 진입 교체에서 범위를 선택한다. 홈·전투 HUD·로비 하위 6종 화면의 이번 복구를 전체 서비스 이관 완료로 취급하지 않는다. `--app` 자동 저장·시작 시 정지 복원은 2단계의 완료된 부분이므로 다시 미착수 작업으로 취급하지 않는다. 강제 종료 시 마지막 저장 이후 모든 프레임의 보존이나 기존 Android 설치 데이터 인계까지 완료한 것은 아니다. 본게임 교체는 각 단계의 완료 조건을 충족한 뒤 진행한다.
...
이 문서는 Godot 전환의 단계별 상태와 완료 조건의 원본이다. 상세 계약과 실행 결과는 해당 문서·검증 기록에 두고, 문서 지도·구현 요약·백로그에는 상태를 복제하지 않고 이 문서를 연결한다.
