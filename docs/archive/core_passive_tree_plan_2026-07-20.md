# 코어 패시브 트리 초기 구현 계획 — 역사 기록

문서 상태: 2026-07-20 인터뷰 기반 구현 계획 원본 보존 (2026-08-06 정정 포함)
보존일: 2026-09-09

현행 참조는 [코어 패시브 트리](../core_passive_tree_implementation_plan.md)를 따른다.
아래 전체 본문은 과거 기록의 인용이다. 구현 명령, 승인 요구, 제외 범위, 미완료 체크리스트를
현재 작업에 적용하지 않는다. 27노드 및 효과 미적용 조건은 현행 계약이 아니다.
인용 안의 코드·문서 경로는 당시 저장소 루트 기준이며, 현재 존재 여부를 보장하지 않는다.
