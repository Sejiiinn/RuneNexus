...
old = json.loads((ROOT / 'docs/analysis/projectile_fps_20260919/projectile-final.json').read_text())
new = json.loads((OUT / 'selection-final.json').read_text())
...
for name in ['normal', 'fire']:
    ...
    layers = []
    text = (OUT / f'selection-{name}-surfaceflinger.txt').read_text()
