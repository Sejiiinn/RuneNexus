...
마지막 정리 기준: `main` 브랜치 커밋 `9f04728` 이후 작업트리.
...
- 스테이지 1과 2는 맵, 웨이브, 해금 보상이 구분됐지만 스테이지 3~5는 아직 스테이지 2의 장갑병 웨이브를 공유한다.
...
### 1. 스테이지 3~5 후속 차별화 범위 결정

가장 먼저 추천하는 작업이다.

작업 후보:

- 스테이지 3~5에 각자 다른 웨이브 압박 추가
- 스테이지별 시작 골드, 보상, 적 구성 차이 추가
- 스테이지 카드에 3~5의 차별점을 짧게 표시
- 현재 스테이지 체력 보정과 새 차별화 요소가 과하게 중첩되지 않는지 검토

이유:

- 스테이지 1과 2의 차별화는 이미 들어갔으므로, 다음 반복 플레이 약점은 3~5가 같은 장갑병 웨이브를 공유한다는 점이다.
- 해금 구조는 이미 있으므로 후속 스테이지를 플레이할 이유를 만드는 효과가 크다.
...
- 스테이지 3~5 차별화와 함께 붙이면 반복 플레이 명분이 강해진다.
...
- 스테이지 3~5 차별화 전후로 재검토하면 과도한 난이도 상승을 줄일 수 있다.
...
```text
docs/next_work_priorities.md 기준으로 1순위인 스테이지 3~5 후속 차별화 범위 결정 작업을 진행해줘.
```
