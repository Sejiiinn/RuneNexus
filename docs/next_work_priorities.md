...
Godot 전환 항목 갱신: 2026-09-24. 그 외 백로그 정리: 2026-09-09. 경제 전환·운영 준비의 낡은 상태를 구현/배포 기록과 정합화했다.
...
전환 상태·완료 조건·다음 구현 범위는 [Godot 단일 앱의 단계별 상태](godot_unified_app_roadmap.md#3-단계와-의존-관계)에서 관리한다. Godot 기본 앱과 `--session` 개발 세션의 검증을 구분하며 이 백로그에 구현 상태를 복제하지 않는다. 웹 배포는 사용자 결정으로 범위에서 제외한다. 아래 콘텐츠 후보는 기존 기능 이관과 별도이며 최신 사용자 요청이 우선한다.
