...
Flame 제거와 Android 전투·3D 표시의 Godot 이관은 완료된 출발점이다. 남은 Flutter 호스트·HUD·로비·Dart 저장/인증 서비스·패키징의 대체 및 삭제 순서는 [Flutter 제거 실행 로드맵](godot_unified_app_roadmap.md)을 따른다. Flutter 전투 호스트 대체는 완료했고 Dart 런·성장·로컬 저장 대체를 진행 중이다. 저장 호환 기반과 독립 개발 세션의 저장 복구를 구현했으며, 본게임 도메인·콘텐츠·기존 설치 인계 연결이 남아 있다. 기존 기능·외형·서버·저장 계약을 보존한다. 아래 신규 기능 백로그와 기존 Flutter 의존성 제거를 구분한다.
