...
- 최우선 과제는 포탑 특성 선택 UX 정리와 저격 포탑 특성 추가다. 현재 특성 시스템은 동작하지만, 2차 특성 탐색성과 선택 확정 흐름이 부족하고 저격 포탑은 특성 선택지가 비어 있다.
- 그 다음 과제는 챕터 2 클리어 보상과 연구 조건 연결이다. 챕터 2는 스테이지 6~10이며, 보호막병 웨이브 MVP와 챕터 2 타일 테마는 해당 구간에 들어갔으므로 이제 반복 동기를 만들어야 한다.
...
### 1. 포탑 특성 선택 UX와 저격 특성 완성

가장 먼저 추천하는 작업이다.
특성 설계 기준은 `docs/gem_shard_trait_design.md`를 따른다.

작업 후보:

- 1차 특성을 선택하지 않아도 2차 특성 목록은 열리게 유지
- 2차 특성 조건을 만족하지 못했거나 1차 특성이 없으면 후보 설명은 보여주되 선택 버튼만 비활성화
- 특성 후보 카드를 처음 누르면 임시 선택/상세 확인 상태가 되고, 같은 후보를 한 번 더 누르거나 명확한 확정 버튼을 눌렀을 때만 선택 완료
- 선택 즉시 확정되는 현재 흐름을 제거해 오선택 가능성을 줄임
- 저격 포탑 1차/2차 특성 선택지를 설계하고 바로 구현
- 저격 특성은 장거리 즉발, 조준 속도, 치명타, 저격 전용 연쇄 beam과 연결되는 방향을 우선 검토
- 위 변경에 맞춰 특성 팝업 테스트와 저장/복구 회귀 테스트 보강

이유:

- 2차 특성을 열어 볼 수 있어야 플레이어가 "어디까지 키울지"를 미리 판단할 수 있다.
- 비활성화와 선택 상태 표시는 잠금 조건을 설명하면서도 빌드 계획을 막지 않는다.
- 특성은 되돌릴 수 없는 선택이므로, 후보를 누르는 즉시 확정되면 UI 실수의 비용이 너무 크다.
- 저격 포탑은 이미 챕터 1 클리어 보상이고 챕터 2에서도 핵심 대응 수단이므로, 특성이 없는 상태를 오래 두면 포탑 간 성장 밀도가 어긋난다.

관련 파일:

- `docs/gem_shard_trait_design.md`
- `lib/domain/turret/turret_trait_catalog.dart`
- `lib/domain/turret/turret_trait_type.dart`
- `lib/game/components/turret_component.dart`
- `lib/game/game_snapshot.dart`
- `lib/data/save/game_save_data.dart`
- `lib/ui/hud/turret_trait_panel.dart`
- `lib/ui/hud/gem_equip_panel.dart`
- `test/turret_trait_test.dart`
- `test/game_balance_test.dart`
- `test/widget_test.dart`

### 2. 챕터 2 보상과 연구 조건 연결

특성 UX와 저격 특성 작업 다음으로 추천하는 작업이다.
...
- 현재 연구 2종의 해금 스테이지, 룬 비용, 연구 시간 검토
...
```text
docs/next_work_priorities.md 기준으로 최우선 과제인 포탑 특성 선택 UX와 저격 특성 완성을 진행해줘.
```
