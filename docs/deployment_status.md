...
## 2026-09-15 APK 0.1.16 / code 6025 배포 준비

사용자가 승인한 미배포 본게임 변경을 포함한다. 배포 완료 여부·공개 자산 값은 workflow 종료 후 갱신한다.

- 화염 포탑 승인 3D 모델, 상부 불꽃·포구·발사체와 적 공통 GPU 화상 효과.
- 6024 이후 커밋된 공통 서리·얼음 결정 및 대포 폭발 파편·불티 GPU 운동 최적화.
- 3D 경로의 기존 2D 화염·냉각 명중 표시와 2D 화상 중복 표시 제거. 피해·지속시간·저장 계약은 유지한다.
- 사거리 원은 선택 포탑만, 설치 패널 활성 중 기존 전체 포탑을 표시한다. 설치 미리보기·업그레이드·보상 우선순위는 유지하고, Godot 투영 좌표를 캐시한다.
- 종료된 화염 입자의 GPU 요청 중단. `ranchu` / `sdk_gphone64_arm64` 에뮬레이터에만 임시 Skia 호환 처리하며, 실기기는 Flutter 기본 렌더러를 유지한다.
- 전체 Flutter 검사 833개 통과·11개 skip, 변경된 Godot 팩 준비 Python 검사 통과. 앞선 본게임 Android 화면·Godot 수명·정지·재사용·사거리 캐시 검사 근거를 유지한다. CI에서도 필수 검사를 실행한다.
- 배포 전 동일 CPU 3종 로컬 APK는 416,018,012 bytes. 기존 공개 6024의 411,353,104 bytes 대비 4,664,908 bytes(약 1.13%) 증가했다. PCK 90,868,632 bytes, 완전 중복 0 bytes. Flutter 원본 GLB·design 제작 파일은 없다. 이 로컬 팩에 있는 미사용 실험 shader 2,280 bytes는 커밋에서 제외하여 CI의 clean checkout 팩에 포함하지 않는다.
- 외형·입자 수·시뮬레이션 주기 유지. 에뮬레이터 1배속 60초 재측정은 화염 전투 갱신 23.01회/초, Flutter raster 평균 24.92ms로 60 FPS 목표 미달이다. 실기기 지속 성능·사용자 기기 설치는 미검증이다.
- 서버·DB·웹 배포와 Flame 전체 제거·Godot 전투 이관은 이번 APK 범위가 아니다. 제작 원본·검증용 앱·미사용 실험 파일은 이번 코드 커밋에 일괄 추가하지 않는다.
