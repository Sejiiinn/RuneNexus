// Local A/B entrypoint. Uses the production GameHud/bridge with memory-only data.
import 'dart:async';
import 'dart:convert';
...
const samples = int.fromEnvironment('RN_PERF_SAMPLES', defaultValue: 15);
...
class _PerformanceRunState extends State<PerformanceRun> {
  ...
  Future<void> _measure(String phase, int warmSeconds) async {
    ...
    rasterUs.clear();
    game.measuring = true;
    record({'event': 'begin', 'phase': phase});
    var collected = 0;
    ...
    while (collected < samples) {
      ...
      record({'event': 'sample', 'phase': phase, 'metrics': metrics});
      collected++;
    }
