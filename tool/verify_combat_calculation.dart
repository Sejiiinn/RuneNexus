import 'dart:convert';
import 'dart:io';
import 'combat/calculation_fixture.dart';
...
Future<void> main(List<String> args) async {
  ...
  final fixturesPath = args.length > 1
      ? args[1]
      : '$root/test/fixtures/combat_calculation.json';
  ...
  try {
    ...
    File(
      '$root/godot/combat/attack_calculation.gd',
    ).copySync('${temp.path}/combat/attack_calculation.gd');
    File(
      '$root/godot/verify_combat_calculation.gd',
    ).copySync('${temp.path}/verify_combat_calculation.gd');
    ...
    final process = await Process.run(godot, [
      ...
      temp.path,
      '--script',
      'res://verify_combat_calculation.gd',
      '--',
      fixturesPath,
      output,
    ]);
    ...
    for (var i = 0; i < fixtures.length; i++) {
      ...
      final dart = evaluateFixture(fixture['input'] as Map<String, dynamic>);
      compareValues(
        dart,
        fixture['expected'],
