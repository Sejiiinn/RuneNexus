...
SOURCE=OUT/'foundry-props-optimized.blend'
ASSET=OUT/'chapter3-props-optimized.glb'
SIZE=1024
...
def main():
    ...
    bpy.ops.wm.save_as_mainfile(filepath=str(OUT/'chapter3-props-optimized-baked.blend'))
    ...
    optimization_path=OUT/'optimization-manifest.json'
    optimization=json.loads(optimization_path.read_text())
    assert hashlib.sha256(Path(optimization['source']).read_bytes()).hexdigest()==optimization['source_sha256']
    assert hashlib.sha256(Path(optimization['baseline_game_asset']).read_bytes()).hexdigest()==optimization['baseline_game_sha256']
    optimization.update({'optimized_glb':ASSET.name,'optimized_glb_sha256':report['glb_sha256'],'glb_bytes':report['glb_bytes'],'total_triangles_before':sum(p['triangles'] for p in optimization['before'].values()),'total_triangles_after':sum(p['triangles'] for p in optimization['after'].values()),'source_and_game_hashes_preserved':True,'export_report':'geometry-check.json','materials':'one shared native PBR material; per-kind one surface;1024 basecolor/normal/ORM/emission;KHR emission strength2.2 preserved','performance_scope':'Geometry and GLB byte reductions only; FPS/GPU performance unmeasured'})
    optimization['triangle_reduction_percent']=100*(1-optimization['total_triangles_after']/optimization['total_triangles_before'])
    optimization['per_kind_reduction_percent']={name:100*(1-optimization['after'][name]['triangles']/part['triangles']) for name,part in optimization['before'].items()}
