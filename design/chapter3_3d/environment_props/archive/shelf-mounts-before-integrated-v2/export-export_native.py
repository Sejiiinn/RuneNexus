...
MOUNTED=OUT.parent/'mounted-scene.blend'
...
def main():
    ...
    mats={}; groups={}; checks={}
    mounted_hash=hashlib.sha256(MOUNTED.read_bytes()).hexdigest()
    with bpy.data.libraries.load(str(MOUNTED),link=False) as (lib,dest):
        dest.objects=[n for n in lib.objects if n.startswith(('External service ledge','Worn service rim','Load-bearing angle bracket','Mount elbow_pipe','Mount exhaust_vent','elbow_pipe mounted','exhaust_vent mounted'))]
    mounted=[o for o in dest.objects if o]
    for name in ('elbow_pipe','side_conduit','exhaust_vent'):
        ...
        if name!='side_conduit':
            # Source ledge local coordinates are tile-root-relative; translate into prop root.
            x=-.25 if name=='elbow_pipe' else .25
            for ob in mounted:
                if ob.type=='MESH' and ob.parent and ob.parent.name.startswith('Mount '+name):
                    from mathutils import Vector
                    transform=Matrix.Translation(Vector((-x,.755,0)))@ob.matrix_basis
                    sources.append((ob,transform))
    ...
    for o in merged:
        ...
        o['mount_back_y']=.135 if o.name=='side_conduit' else .29;o['surfaceZ']=0.;o['source']=str(SOURCE.relative_to(ROOT))
    ...
    assert hashlib.sha256(MOUNTED.read_bytes()).hexdigest()==mounted_hash
    report={'source_sha256':source_hash,'source_unchanged':True,'tiles':checks,'reimport':reimport,'atlas_size':SIZE,'channels':list(images),'glb_bytes':ASSET.stat().st_size,'glb_sha256':hashlib.sha256(ASSET.read_bytes()).hexdigest(),'mounted_sha256':mounted_hash,'source_features':'3 approved props; service ledge and 2 support brackets included for elbow/exhaust; hollow bores and recessed orange volumes preserved'}
