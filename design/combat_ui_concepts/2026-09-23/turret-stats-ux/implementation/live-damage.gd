...
func run():
	...
	var map=app.catalog.stage(0).map
	var tile=map.tiles.find("build")
	app.board_tap(Vector2i(tile % int(map.columns),tile / int(map.columns)));app.build_selected()
	state=app.run_domain.state;turret=state.turrets[0]
	hud.tab="stats";hud.main_tab="turrets";await refresh()
	var old_body=hud.body.find_child("TurretStatsGrid",true,false)
	var old_label=hud.damage_label
	var id=str(turret.id)
	var runtime=app.scene._native_combat
	var actual=runtime.turrets[id]
	actual.directDamageDealt=12.5;actual.splashDamageDealt=20.0;actual.chainDamageDealt=30.0;actual.burnDamageDealt=40.0
	hud.refresh()
	check(hud.damage_label.text=="102.5","live sum direct+splash+chain+burn")
	check(hud.damage_label==old_label and hud.body.find_child("TurretStatsGrid",true,false)==old_body,"live damage avoids rebuilding body")
	actual.burnDamageDealt=50.5;hud.refresh()
	check(hud.damage_label.text=="113.0","live cumulative updates nonzero")
	await capture("live-damage-440")
	print("LIVE_DAMAGE ",JSON.stringify({"checks":checks,"failures":failures}))
