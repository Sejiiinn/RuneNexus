...
var app
var lobby
var out := "/Users/sejin/Documents/Codex/RuneNexus/design/combat_ui_concepts/2026-09-22/stage-modal-proposals/verification/"
...
func run():
 ...
 for stage in [8,11,1]:
  ...
  root.content_scale_size=Vector2i(320,760);root.size=Vector2i(320,900)
  await create_timer(0.4).timeout;check(stage)
  for name in lobby.modal_body.find_children("RewardName","Label",true,false):
   assert(name.get_theme_font_size("font_size")==16)
   assert(name.get_line_count()<=2,"reward name too many lines "+name.text)
   assert(lobby.modal_scroll.get_global_rect().encloses(name.get_global_rect()),"reward name clipped")
  await RenderingServer.frame_post_draw
  root.get_texture().get_image().save_png(out+"strict-real-320-stage"+str(stage)+".png")
  root.content_scale_size=Vector2i(440,760);root.size=Vector2i(440,900)
  await create_timer(0.4).timeout;check(stage)
  var action=lobby.modal_body.find_child("StageAction",true,false)
  if stage==8:
