...
현재 출력은 두 메시·두 재질, **23,964삼각형**(식생 20,496·정적 3,468), **1,552,836바이트**다. 24,000삼각형 예산 안이며 텍스처·스킨·애니메이션 클립·물리·추가 광원을 포함하지 않는다. 식생은 기존 조명을 받고 추가 그림자를 생성하지 않는다. 새 장식 GLB는 공용 Godot PCK에만 포함하며 Flutter 자산과 중복 패키징하지 않는다. `design/` 원본과 제작 렌더는 게임 패키지에 포함하지 않는다.
