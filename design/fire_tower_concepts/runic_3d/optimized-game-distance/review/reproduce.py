...
with tempfile.TemporaryDirectory(prefix="runenexus-fire-s26-qhd-") as directory:
    baseline = Path(directory) / "original-magic.glb"
    data = subprocess.check_output(["git", "show", "08110e36508544efbc643f2f5e3ba515bba556a7:assets/images/stage1_3d/turrets/magic.glb"], cwd=repo)
    assert hashlib.sha256(data).hexdigest() == "80be26ff3b5932193746df410e513290932ec4687f9f5eb5efed3cf39addd380"
    ...
    subprocess.run([str(godot), "--path", str(project), "--resolution", "440x760",
                    ...
                    "--original=" + str(baseline), "--optimized=" + str(Path(sys.argv[1]).resolve())], check=True)
