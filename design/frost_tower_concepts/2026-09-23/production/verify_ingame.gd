...
var output := "/Users/sejin/Documents/Codex/RuneNexus/design/frost_tower_concepts/2026-09-23/production/godot"
...
func run() -> void:
	...
	check(OS.get_user_data_dir().contains("RuneNexus-Frost-Review"), "isolated user directory")
	...
	# 실제 전투 시계로 발사·감속, 조준·반동 후 연결을 확인한다.
	app.hud.show()
	scene.options.camera = "angled"
	scene._apply_options()
	if scene.camera_transition and scene.camera_transition.is_running(): await scene.camera_transition.finished
	...
	for tick in range(900):
		...
		for turret in scene._native_combat.turrets.values():
			if turret.statInput.definition.type == "frost" and turret.shotSequence > 0: fired = true
		for enemy in scene._native_combat.enemies.values():
			if not enemy.get("slowInstances", []).is_empty(): slowed = true
		if fired and tick > 240: break
	app._process(0.0)
	check(fired, "frost fired through existing combat path")
	await capture("app-combat")
	print("FROST_REVIEW fired=", fired, " slowed_observed=", slowed, " failures=", failures)
	var report := {"engine":Engine.get_version_info(), "renderer":RenderingServer.get_current_rendering_method(), "user_directory":OS.get_user_data_dir(), "asset_sha256":FileAccess.get_sha256("res://assets/turrets/frost.glb"), "fired":fired, "slowed_observed":slowed, "failures":failures}
