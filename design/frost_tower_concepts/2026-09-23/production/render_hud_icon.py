...
WORK=ROOT/'design/frost_tower_concepts/2026-09-23/production'
OUT=WORK
KINDS=['frost']
# Preserve every other open scene and original file.
records={}
for kind in KINDS:
    scene=bpy.data.scenes.new('HUD_'+kind)
    bpy.context.window.scene=scene
    source=WORK/'frost.glb'
    ...
    scene.render.filepath=str(OUT/'hud-frost.png')
    bpy.context.view_layer.update()
...
bpy.context.window.scene=scene
bpy.data.libraries.write(str(WORK/'hud-frost.blend'),{scene},fake_user=True,compress=True)
(WORK/'hud-direction-verification.json').write_text(json.dumps(records,indent=2))
