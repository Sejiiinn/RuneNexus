...
var output := "/Users/sejin/Documents/Codex/RuneNexus/design/frost_tower_concepts/2026-09-23/optimized-game-distance/integration"
...
func capture(name: String) -> void:
	if "--structure-only" in OS.get_cmdline_user_args() and name != "structure-low-angle": return
...
func run() -> void:
	...
	check(OS.get_user_data_dir().contains("RuneNexus-Frost-Optimized-Integration"), "isolated user directory")
	...
	for startup_frame in 120:
		if app != null and not app.startup_blocked: break
		await process_frame
	...
	if "--structure-only" in OS.get_cmdline_user_args():
		scene.options["turret_levels"] = false
		scene._turret_level_labels.visible = false
		scene.camera.position = center + Vector3(4, 2.6, 6.4)
		scene.camera.look_at(center)
		scene.camera.near = .05
		scene.camera.far = 100
		await capture("structure-low-angle")
		print("FROST_STRUCTURE_CAPTURE_COMPLETE")
		scene.queue_free()
	...
	for tick in range(900):
		...
		if fired:
			await process_frame
			await RenderingServer.frame_post_draw
			pass # no duplicate full video for this local mesh refinement
			video_frame += 1
	...
	# Low-angle structure evidence with the unchanged production model.
	scene.camera.position = center + Vector3(1.0, .65, 1.6)
	scene.camera.look_at(center)
	await capture("structure-low-angle")
	app.hud.show()
	scene._apply_options()
