...
func _verify() -> void:
	...
	var mask_updates: int = overlay._mask_updates
	var static_redraws: int = overlay._static_redraws
	overlay.present(camera, Vector2(8, 8), world)
	_check(overlay._mask_updates == mask_updates, "Unchanged reward mask rendered again")
	_check(overlay._static_redraws == static_redraws, "Unchanged projection rebuilt static commands")
	_check(overlay._mask_viewport.render_target_update_mode != SubViewport.UPDATE_ALWAYS, "Mask keeps rendering without changes")
	var cached: PackedVector2Array = overlay._range_path(Vector2(4,4), 1.3)
	overlay._tile_path(Vector2(4,4), 0.04, 0.09)
	overlay._scale_at(Vector2(4,4))
	overlay.prepare_context(camera, Vector2(8,8), world)
	_check(overlay._range_paths.size() > 0 and overlay._tile_paths.size() > 0 and overlay._scales.size() > 0, "Stable context discarded geometry cache")
	var arc: PackedVector2Array = overlay._path(Vector2(4,4), 0.43, 1.2, PI*1.64)
	for i in range(arc.size()):
		var angle := 1.2 + PI*1.64*float(i)/(arc.size()-1)
		_check(arc[i].distance_to(overlay._project(Vector2(4,4)+Vector2(cos(angle),sin(angle))*0.43)) < 0.001, "Cached arc changed tessellation")
	mesh.mesh.size.x += 0.1
	overlay.present(camera, Vector2(8,8), world)
	_check(overlay._mask_updates > mask_updates, "Mesh resource edit left stale silhouette")
	...
	camera.look_at(Vector3.ZERO)
	overlay.present(camera, Vector2(8, 8), world)
	_check(overlay._range_paths.is_empty() and overlay._tile_paths.is_empty() and overlay._scales.is_empty(), "Camera change retained stale projected geometry")
	...
	mesh.show()
	overlay.present(camera, Vector2(8,8), world)
	_check(overlay._mask_meshes.size() == 1, "Target visibility restore lost mask")
	var extra := MeshInstance3D.new()
	extra.mesh = BoxMesh.new()
	model.add_child(extra)
	overlay.present(camera, Vector2(8,8), world)
	_check(overlay._mask_meshes.size() == 2, "Target hierarchy addition absent from cached mask")
	extra.free()
	overlay.present(camera, Vector2(8,8), world)
	_check(overlay._mask_meshes.size() == 1, "Deleted target mesh retained cached mask")
	overlay.hide()
	_check(overlay._mask_meshes.is_empty(), "Hidden overlay retained active mask")
	overlay.show()
	overlay.present(camera, Vector2(8,8), world)
	_check(overlay._mask_meshes.size() == 1, "Visible overlay did not restore mask")
